...
import { Lexer } from "../../../utilities/lexer/lexer";
import { Assert } from "../../framework/assert";

export class AssertLexer {
    static CanParse(lexer: Lexer, ...scripts: string[]){
    ...
    static CannotParse(lexer: Lexer, ...scripts: string[]) {
        scripts.forEach(script => {
            Assert.IsFalse(lexer.TryParse(script));
        });
    }

    static IsParseMatches(lexer: Lexer, parse: ExpressionTreeNode) {
